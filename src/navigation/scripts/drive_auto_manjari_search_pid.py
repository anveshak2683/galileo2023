...
#                self.write_coordinates()
    ...
    def rotate(self,dir):
        ...
        msg.vel = 127
        msg.hb=False
        diff  = self.z_angle - self.initial_yaw     # diff should ideally be between [-90,90], so the next if-elif is just in case (it shouldn't enter it)
        if (diff > 120):
            diff = diff - 360
        elif (diff < -120):
            diff = diff + 360
        ...
        '''
        error = self.rotate_angle-abs(diff)     #rotate_angle is 90 degrees, set in constructor
        print("error=", error)
        if (abs(error)>self.angle_thresh):      #angle_thresh is 3, set in constructor
            msg.omega=127+(dir*35)
            msg.vel=115
